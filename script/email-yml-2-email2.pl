...
sub main {
    ...
    for my $h1(keys %$old_data) {
        if ($h1 eq 'blocked_email') {
            ...
            $new_data->{keep} = {expiration_days=>10,whitelist=>1,criteria=>[]};
            for my $body_like(@{ $old_data->{$h1} }) {
            ...
            for my $folder (keys %{ $old_data->{$h1} }) {
                ...
                $new_data->{$name} = {expiration_days=>9,move_to=>"INBOX.$folder",criteria=>[]};
        ...
        elsif ($h1 eq 'banned_ip_slices') {
            $new_data->{blocked} = {expiration_days=>0,move_to=>$spam,criteria=>[]} if ! exists $new_data->{blocked};
            for my $h2 (@{ $old_data->{$h1} }) {
                push @{ $new_data->{blocked}->{criteria},  }, {ip_address_in => $$h2};
            }
        }
